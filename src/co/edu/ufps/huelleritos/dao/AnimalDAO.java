...
public List<Animal> listAdoptado(){
	TypedQuery<Animal> consulta= em.createNamedQuery(Animal.class.getSimpleName()+".findAllAdoptado", Animal.class);
	List<Animal> lista = (List<Animal>) consulta.getResultList();
	return lista;
}
...
public List<Animal> listAnimalesByUsuario(String usuario) {
	...
	return animalesEncontrados.stream().map(animal -> new Animal((String)animal[0],(String)animal[1],(String)animal[2])).collect(Collectors.toList());
}
